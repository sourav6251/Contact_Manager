...
import { Settings, User, Lock, Smartphone, ChevronRight ,CheckCircle} from "lucide-react";
 ...
 >
   ...
   {menuItems.map((item) => (
     <option key={item.id} value={item.id}>
      ...
      <select className="w-full py-3 pl-4 pr-10 rounded-xl border dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
        value={selectedOption || ""}
        onChange={(e) => setSelectedOption(e.target.value || null)}>
        ...
        {menuItems.map((item) => (
          ...
          >
            <span className={`inline-block mr-2 ${item.color}`}>
              {item.icon}
            </span>
            {item.title} 
            {item.description && (
    ...
    {/* Optional: Quick action buttons for mobile */}
    {selectedOption && (
      <motion.div 
        initial={{ opacity: 0, y: -10 }}
        animate={{ opacity: 1, y: 0 }}
